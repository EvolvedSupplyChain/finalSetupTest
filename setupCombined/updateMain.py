...
def updater():
    ...
    for index, files in enumerate(config['FILEMANIFEST']):
        ...
        #if ota_updater.filename == "config.json":
        #if the file being updated is config.json, merge existing provisioning info with new manifest, etc.  Unless clobber flag is true (FORCECONFIG)  
